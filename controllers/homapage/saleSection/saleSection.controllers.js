...
const registerSaleSection = async (req, res) => {
    try {
        ...
        const saleScetion = new SaleSection({ image: images }); // create sale section
        await saleScetion.save();

        if (!saleScetion) {
            return res.status(401).json({ Message: "images not saved in database" }); // check sale section
        }
        return res.status(200).json({ Message: "Sales section has been created", sale_Section: saleScetion }); // return response
    } catch (error) {
